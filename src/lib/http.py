...
# Abort a run when this share of *calls* is anomalous (spec §6.4).
#
# Measured over distinct calls, not HTTP attempts. Counting attempts makes one bad URL look
# like three failures, which is how the first live run aborted at 10.7% on a single category
# page that is simply empty (Play's MEDICAL category serves a 1 MB page with no app links,
# while every neighbouring category works).
ANOMALY_ABORT_RATE = 0.10
ANOMALY_MIN_SAMPLE = 25

# A burning IP looks like *many different* URLs failing in a row. One URL failing repeatedly
# is a content problem. This second signal catches genuine degradation early even when the
# rate is still diluted by earlier successes.
...
class FetchStats:
    ...
    Two levels of counting, and the distinction matters:
      * `requests` — HTTP attempts, retries included. What the store's rate limiter sees.
      * `calls`    — distinct things we tried to fetch. What the health signal is measured
                     over, so a single unlucky URL cannot masquerade as a failing IP.
    """
    ...
    def anomaly_rate(self) -> float:
        """Share of distinct calls that failed outright — the IP-health signal."""
    ...
    def __str__(self) -> str:
        return (
            f"{self.calls} calls ({self.requests} req), {self.failed_calls} failed, "
            f"403={self.http_403} 429={self.http_429} 5xx={self.http_5xx} "
            f"empty200={self.empty_200} err={self.errors} "
            f"({self.anomaly_rate:.1%} of calls anomalous)"
        )
...
class Fetcher:
    ...
    def _record_call(self, ok: bool) -> None:
        """Close out one distinct call and re-evaluate IP health.

        Called once per get()/guarded(), after retries are exhausted — so a URL that fails
        three times counts as one failure, not three.
        """
        self.stats.calls += 1
        if ok:
            self._consecutive_failures = 0
    ...
    def _check_degradation(self) -> None:
        ...
        if self._consecutive_failures >= CONSECUTIVE_FAILURE_LIMIT:
            raise Blocked(
                f"{self._consecutive_failures} consecutive calls failed — that is a "
                f"degrading IP, not a bad URL. {self.stats}"
            )

        if self.stats.calls >= ANOMALY_MIN_SAMPLE and self.stats.anomaly_rate > ANOMALY_ABORT_RATE:
            raise Blocked(
                f"{self.stats.failed_calls}/{self.stats.calls} calls failed "
                f"({self.stats.anomaly_rate:.1%}), over {ANOMALY_ABORT_RATE:.0%} — "
    ...
    def get(
        ...
        for attempt in range(limit + 1):
            ...
            try:
                ...
                self._backoff(attempt, limit, type(e).__name__)
                continue
            self._last_request_at = time.monotonic()
            ...
            if r.status_code == 200:
                ...
                self._backoff(attempt, limit, "200-but-empty")
                continue

            if r.status_code == 404:
                # A definitive "not here" from the server, not a failure of ours. Callers
                # that can distinguish (developer_crawl tries the other URL form) handle it.
                self._record_call(True)
                return None
            ...
            self._backoff(attempt, limit, f"HTTP {r.status_code}")
    ...
    def guarded(self, fn: Callable, *args, **kwargs):
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                # A definitive answer from the store, not a fetch failure.
                self._last_request_at = time.monotonic()
                self._record_call(True)
                ...
                # network fault — retrying cannot help, and it says nothing about the IP.
                self._last_request_at = time.monotonic()
                self._record_call(True)
                ...
                self.stats.errors += 1
                self._last_request_at = time.monotonic()
                self._backoff(attempt, self.max_retries, type(e).__name__)
                continue
            self._last_request_at = time.monotonic()
            self._record_call(True)
            return result
    ...
    def _backoff(self, attempt: int, limit: int, why: str) -> None:
        if attempt >= limit:
            return
        wait = (2**attempt) * 3 + random.uniform(0, 2)
        print(f"    retry {attempt + 1}/{limit} after {why} — {wait:.1f}s")
        time.sleep(wait)
